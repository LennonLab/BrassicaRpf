...
2) How does Rpf alter the soil microbial abundance, activity, and community structure?

Data Analysis:

1a) Performed t-test to test Rpf effects on plant traits (i.e., seed number, biomass, SLA, height).
2a) Performed repeated-measures ANOVA (RM-ANOVA) to test Rpf effects on soil respiration.
2b) Performed t-test to test Rpf effects on soil microbial abundance (i.e., bacterial and fungal).
2c) Generated a PCoA and performed PERMANOVA to test Rpf and metabolic status effects on soil bacterial community composition. 
...
# p-value label 
mtext(text = expression(italic("P")~" = .097") , side =3, line = -1.2, adj = 0.925, col="black", cex=0.8)
...
co <- ggplot(CO2.means.sem, aes(x=Week, y=mean, colour=Treatment, group=Treatment)) +
      ...
      xlab("Time (weeks)") +
      ylab(expression(Soil~respiration~(ppm~CO[2]~d^-1~g^-1~soil))) 
...
# Load packages # 
package.list <- c('vegan', 'plyr', 'grid', 'png', 'picante', 'ade4',  'phangorn', 'indicspecies', 'BiodiversityR') 
for (package in package.list) {
  if (!require(package, character.only=T, quietly=T)) { 
    install.packages(package)
    library(package, character.only=T)
...
act.bray.permanova <- adonis(Actino.OTU.REL ~ Brassica.div$type + Brassica.div$treatment + Brassica.div$pot + 
...
# Relative abundance of the indicator species with Rpf treatment
# Subset Class Thermoleophilia
Thermo.tax <- OTU.tax[ which(OTU.tax$Class == 'Thermoleophilia'), ]

# Subset Actino.OTU.REL.Log to contain only Thermoleophilia
Thermo.OTU <- match(Thermo.tax$OTU, colnames(Actino.OTU.REL.log))
Thermo.OTU <- sort(c(Thermo.OTU-1, Thermo.OTU))
Thermo.OTU <- Actino.OTU.REL.log[, Thermo.OTU]

# Subset between Rpf treatments and calculate means of sum counts
# +Rpf 
Thermo.Rpf <- Thermo.OTU[c(5:10, 15:20), ]
Thermo.Rpf.count <- as.vector(rowSums(Thermo.Rpf))
mean(Thermo.Rpf.count)
# -Rpf 
Thermo.Con <- Thermo.OTU[c(1:5, 10:15), ]
Thermo.Con.count <- as.vector(rowSums(Thermo.Con))
mean(Thermo.Con.count)


# T-test for effect of RPf 
Gram.Ratio.ttest <- t.test(Thermo.Con.count, Thermo.Rpf.count)
Gram.Ratio.ttest
```
...
# Plotting Gram Ratio # 
png(filename="../figures/SupplementaryFig2-GramRatio.png",
...
# Show Plot
img <- readPNG("../figures/SupplementaryFig2-GramRatio.png")
grid.raster(img)
...
## Actinobacteria: Everything else ratios
...
# Plotting Gram Ratio # 
png(filename="../figures/SupplementaryFig3-ActinoToEverythingRatio.png",
    width = 800, height = 800, res = 96*2)
par(mar = c(5, 5, 1, 1))
...
# Show Plot
img <- readPNG("../figures/SupplementaryFig3-ActinoToEverythingRatio.png")
grid.raster(img)
