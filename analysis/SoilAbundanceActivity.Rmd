...
package.list <- c('plyr', 'grid' ,'png', 'car', 'bbmle', 'ggplot2', 'vegan', 'tidyr', 'dplyr', 'codyn', 'cowplot', 'MullerPlot', 'RColorBrewer', 'pander', 'reshape') 
...
bac.means.barplot + geom_errorbar(aes(ymax=upper, ymin=lower),
  ...
  labs(x="Week of growth chamber experiment", y="16S rRNA gene copy number/Soil (g)") +
  theme_classic() +
  ...
  annotate("text", x =0.60, y=75000, label = c("A"), size=12)
...
ITS.mean.barplot + 
  ...
  labs(x="Week of growth chamber experiment", y="ITS gene copy number/Soil (g)") +
  theme_classic() +
  scale_fill_manual(values=c("Black", "White")) +
  theme(axis.text.y=element_text(colour="black", size=16),
        axis.text.x=element_text(colour="black", size =16),
        ...
        legend.title = element_text(size=18),
        legend.text=element_text(size=18)) +
  annotate("text", x =0.55, y=1000000, label = c("B"), size=12)
```
...
bac.aov <- anova(lm(stdGeneCopy ~ Treatment*Week, data=bac.abundance))
bac.aov
...
ITS.aov <- anova(lm(stdGeneCopy ~ Treatment*Week, data=ITS.abundance))
ITS.aov
# Results: Rpf application to potted soil significantly (p<0.005) reduces bacterial 16S rRNA gene copy numbers # 
# Gene copy number increases significantly over the 6 week growth chamber experiment # 
```
...
ggplot(CO2.means, aes(x=variable, y=mean, colour=Treatment, group=Treatment)) + 
  geom_errorbar(aes(ymax=upper, ymin=lower), position=position_dodge(0), 
    ...
    xlab("Week of growth chamber experiment") +
