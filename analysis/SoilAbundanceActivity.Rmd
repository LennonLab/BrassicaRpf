...
package.list <- c('plyr', 'grid' ,'png', 'plyr', 'car', 'bbmle') 
...
# Read csv files # 
abundance.raw <- read.csv("qPCR.txt", sep = "\t", header=T) 

# abundance.raw[,x] = copies not corrected by dilution factor
# abundance.raw[,8] = dilution factor
# abundance.raw[,9] = volume (uL) in supernatant of phenol-chloroform extraction
# abundance.raw[,10] = volume (ul) from supernatant of phenol-chlorofom subsampled

copies.corr <- abundance.raw[,7] * (abundance.raw[,8] * (abundance.raw[,9]/abundance.raw[,10]))


# Log transformed and standardized based on soil gram # 

CO2 <- read.csv("GCH_CO2.txt", sep = "\t" , header=T)
...
CO2.sub <- subset(CO2, plant=="Present" & hour =="48" & soil=="Sterilized", select=c(Treatment, soil, Week.1, Week.2, Week.3, Week.4, Week.5, Week.6))

# Melt data into three columns of treatment, soil and weeks # 
CO2.m <- melt(CO2.sub)

# Summarizing data # 
CO2.means <- ddply(CO2.m, c("Treatment", "variable"), summarise, mean=mean(value))

# Calculating standard error of the means # 
CO2.means.sem <- ddply(CO2.means, c("Treatment", "variable"), summarise,
                   mean=mean(mean), sem=sd(mean)/sqrt(length(mean)))

CO2.means.sem <- transform(CO2.means.sem, lower=mean-sem, upper=mean+sem)

# Plot line graph of summarized data # 
resp.line <- ggplot(CO2.means, aes(x=variable, y=mean, color=Treatment)) + 
...
ggplot(CO2.means, aes(x=variable, y=mean, colour=Treatment, group=Treatment)) + 
  geom_errorbar(aes(ymax=upper, ymin=lower), position=position_dodge(0.65), 
                data=CO2.means.sem, width = 0.2, size=1.5, colour="black") +
...
# Make line graph of all factors # 

```


## 7) Time series analysis of CO2 respiration across growth chamber experiment 
...
```{r}
# Require or install packages #
package.list <- c('vegan', 'tidyr', 'dplyr', 'codyn', 'ggplot2', 'cowplot', 'MullerPlot', 'RColorBrewer', 'pander') 
for (package in package.list) {
  if (!require(package, character.only=T, quietly=T)) { 
    install.packages(package)
    library(package, character.only=T)
...
# Constructing time-by-Co2 respiration matrix #
#time.by.CO2 <- group_by(CO2.m, variable, soil, Treatment) %>% count(value) %>% spread(key = variable, value) 

# Filter to pick plot #
#resp <- filter(time.by.spec.2, plot_id == 2) %>%
...
#  summarise (mean = mean(richness), sd = sd(richness), n = n(),
#             sem = sd/sqrt(n))
