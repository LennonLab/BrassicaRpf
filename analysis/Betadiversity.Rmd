...
OTUs <- OTUs[1:20, ]   # First 20 samples
```
...
```{r}
# Create factor vector in Rpf treatment 
treatment <- c(rep("Active Rpf-",5), rep("Active Rpf+", 5), rep("Total Rpf-", 5), rep("Total Rpf+", 5))
activity <- c(rep("Active", 5), rep("Total", 5), rep("Active", 5), rep("Total", 5))
adonis(OTUs ~ activity, method="bray", permutations=999)
# Result: Rpf treatment and activiy is significant, p-value < 0.001, in structuring soil microbial community # 

# Multi level pattern analysis # 
indval <- multipatt(OTUs, cluster = treatment, func = "IndVal.g", control = how(nperm=999))
summary(indval)
```
