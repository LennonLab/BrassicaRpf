...
# Load dependencies # 
package.list <- c('vegan', 'plyr' , 'car', 'grid', 'png', 'ape', 'picante', 'ade4', 'phytools', 'phangorn',  'BiodiversityR') 
...
.shared and .taxonomy file are outputs of alignment and clustering programing mothur. 
...
## Richness: does Rpf treatment "inflate" diversity?

```{r}
# Take averages of technial reps
rich.rep.ave <- ddply(Brassica.div, .(type, treatment, rep.num),
...
rich.2.ag.mean.control <- aggregate(rich.cDNA ~ treatment, rich.2, mean)
rich.2.ag.sem.control <- aggregate(rich.cDNA ~ treatment, rich.2, sem)
rich.2.ag.mean.treatment <- aggregate(rich.DNA ~ treatment, rich.2, mean)
rich.2.ag.sem.treatment <- aggregate(rich.DNA ~ treatment, rich.2, sem)

# Calculate ratios
rich.2$rich.ratio <- rich.2$rich.cDNA / rich.2$rich.DNA

# Richness table
rich.2.ag.mean <- aggregate(rich.ratio ~ treatment, rich.2, mean)
rich.2.ag.sem <- aggregate(rich.ratio ~ treatment, rich.2, sem)
rich.2.ag.95 <- aggregate(rich.ratio ~ treatment, rich.2,
                  FUN = function(x) t.test(x)$conf.int[1:2])
rich.table <- data.frame(rich.2.ag.mean$treatment, rich.2.ag.mean$rich.ratio,
                         rich.2.ag.sem$rich.ratio, rich.2.ag.95$rich.ratio)
colnames(rich.table) <- c("treatment", "mean", "sem", "LCI", "UCI")

# One-factor ANOVA 
rich.reg <- lm(rich.2$rich.ratio ~ rich.2$treatment)
Anova(rich.reg)
ttest(rich.reg, 1, 1)
ttest(rich.reg, 2, 0)

# Results: Rpf treatment appears to marginally increase active microbial diversity but this was not statistically significant p= 0.1163 
```
...
## Evenness: does Brassica "inflate" diversity?

```{r}
# Take averages of technial reps
even.rep.ave <- ddply(Brassica.div, .(type, treatment, rep.num),
...
# Calcualte means +/- SEM of control and treated evenness
even.2.ag.mean.control <- aggregate(even.cDNA ~ treatment, even.2, mean)
even.2.ag.sem.control <- aggregate(even.cDNA ~ treatment, even.2, sem)
even.2.ag.mean.treatment <- aggregate(even.DNA ~ treatment, even.2, mean)
even.2.ag.sem.treatment <- aggregate(even.DNA ~ treatment, even.2, sem)

# Calculate ratios
even.2$even.ratio <- even.2$even.cDNA / even.2$even.DNA

# evenness table
even.2.ag.mean <- aggregate(even.ratio ~ treatment, even.2, mean)
even.2.ag.sem <- aggregate(even.ratio ~ treatment, even.2, sem)
even.2.ag.95 <- aggregate(even.ratio ~ treatment, even.2,
                  FUN = function(x) t.test(x)$conf.int[1:2])
even.table <- data.frame(even.2.ag.mean$treatment, even.2.ag.mean$even.ratio,
                         even.2.ag.sem$even.ratio, even.2.ag.95$even.ratio)
colnames(even.table) <- c("treatment", "mean", "sem", "LCI", "UCI")

# One-factor ANOVA 
even.reg <- lm(even.2$even.ratio ~ even.2$treatment)
Anova(even.reg)
ttest(even.reg, 1, 1)
ttest(even.reg, 2, 0)

# Results: Rpf treatment did not affect active or total community evenness. 
```
...
# Rpf treatment affect, but not signifcantly, the microbial community (p=0.074) 
```
...
OTU.REL.a <- OTU.REL.log[which(Brassica.div$type == "cDNA"), ]
OTU.REL.t <- OTU.REL.log[which(Brassica.div$type == "DNA"), ]

# Subset the Design Matrix
Brassica.div.a <- Brassica.div[which(Brassica.div$type == "cDNA"), ]
Brassica.div.t <- Brassica.div[which(Brassica.div$type == "DNA"), ]

# Calculate Bray-Curtis Distances
dist.a <- vegdist(OTU.REL.a, "bray")
dist.t <- vegdist(OTU.REL.t, "bray")

# Run Mantel Test of Distance Matrix
mantel.rtest(dist.a, dist.t, nrepet = 999)
# Results: Treatment did not have a effect on soil microbial community
```
