...
flower.live.aov <- aov(flower.count ~ Treatment, data=plant.sub)
...
# Calculating standard error of means for total plant biomass #
plantmeans.sem <- ddply(plant.m.sub, c("Treatment", "variable"), summarise,
                   mean=mean(value), sem=sd(value)/sqrt(length(value)))
plantmeans.sem <- transform(plantmeans.sem, lower=mean-sem, upper=mean+sem)

# Plotting stacked bargraph # 
live.biomass.stacked.bar  + geom_errorbar(aes(ymax=upper, ymin=lower),
                              position=position_dodge(0), data=plantmeans.sem, width = 0.2, size=1.5) +
...
# One-factored ANOVA testing the effect of treatment on seed counts of plants in live soil # 
total.live.aov <- aov(total.biomass ~ Treatment, data=plant.sub)
...
abiomass.aov <- aov(abiomass ~ Treatment, data=plantfitness)
