...
CN.means.sem <- ddply(CN, c("Treatment", "Week"), summarise,
                   mean=mean(RelN), sem=sd(RelN)/sqrt(length(RelN)))
CN.means.sem <- transform(CN.means.sem, lower=mean-sem, upper=mean+sem)

# Bargraph #
CN.barplot<- ggplot(CN.means.sem, aes(x=Week,y = mean, fill=Treatment)) + 
  geom_bar(width=0.5, colour="black", position = position_dodge(width = 0.65), stat = "identity", size=1.5)  

# Clean Bargraph figure #
CN.barplot + geom_errorbar(aes(ymax=upper, ymin=lower),
                              position=position_dodge(0.65), data=CN.means.sem, width = 0.2, size=1) +
  labs(x="Week of Growth Chamber Experiment", y="Soil Nitrogen (mg)") +
  theme_classic() +
...
```{r}
# Two factor anova of relative soil N concentration # 
anova <- aov(CN$RelN ~ CN$Treatment*CN$Week, data = CN)
summary(anova) 
TukeyHSD(anova) 

# Two factor ANOVA of ratio CN #
anova <- aov(CN$Ratio ~ CN$Treatment*CN$Week, data = CN)
summary(anova) 
TukeyHSD(anova) 
