...
resp.line <- ggplot(CO2.means, aes(x=variable, y=mean, color=Treatment)) + 
    geom_errorbar(aes(ymin=mean-se, ymax=mean+se), data=CO2.means.sem, width=.075) +
    geom_line() +
...
bac.95.LL <- aggregate(stdGeneCopy ~ Treatment, data.sub.bac,
