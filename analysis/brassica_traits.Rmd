...
flower.mean.barplot + 
  ...
  theme(axis.text.y=element_text(colour="black", size=18),
        ...
        axis.ticks.length = unit(.25, "cm"),
        axis.ticks = element_line(size = 2)) +
 annotate("text", x =0.575, y=10.5, label = c("A"), size=12)
```
...
#flower.95.LL <- aggregate(flower.count ~ Treatment, data,
...
mtext(expression('Flower Number / Plant'), side = 2,
...
text(0.7,14 ,labels="A", col="black", cex=2)
...
# Show Plot
img <- readPNG("../figures/Figure1-Flower.count.png")
grid.raster(img)
```
...
```{r}
# Generating figure # 
png(filename="../figures/Figure2-Plantbiomass.png",
    width = 800, height = 800, res = 96*2)

par(mar = c(5, 5, 1, 1))

biomass.fig <- plot(jitter(rep(1, length(root.rpf$value)), amount = 0.1), root.rpf$value, 
      ylim = c(0, 1.5), xlim = c(0.5, 4.5), pch = 21, col = "grey25", bg = "grey25", lwd = 3.5,
      cex = 1.7, yaxt = "n", xaxt = "n", cex.lab = 2, cex.axis = 2,
      las = 1, ylab = "", xlab = "")
      box(lwd = 2)
...
#abline(v=2.5, col="black", lty= 2, lwd = 2)
...
# Show Plot
img <- readPNG("../figures/Figure2-Plantbiomass.png")
grid.raster(img)
```
